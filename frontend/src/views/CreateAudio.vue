<template>
  <div class="create-audio-page">
    ...
    <form @submit.prevent="generateAudio">
      ...
      <div>
        <p class="mb-1" :class="{ 'text-danger': isTokenCountOverLimit() }">
          Input tokens: {{ getInputTokenCount() }} / {{ MAX_TOKEN_COUNT }}
        </p>
    ...
    <div v-if="generatedWav" class="mt-3">
      ...
      Real input tokens: {{ generatedMetadata.inputTokenCount }}, Real output tokens:
      {{ generatedMetadata.outputTokenCount }}, Audio duration:
      {{ formatDuration(generatedMetadata.audioDuration) }}, Generated in:
      {{ formatDuration(generatedMetadata.generationDuration) }}
      ...
      <div>
        <button @click="saveToStorage" class="btn btn-success me-3" type="button">
          Save to storage
        </button>
