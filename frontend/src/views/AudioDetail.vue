<template>
  <div class="create-audio-page">
    ...
    <div v-if="idNotFound" class="bg-danger text-white p-3">
      Cannot find audio with ID {{ audioId }}.
    </div>

    <div v-else>
      <p>
        Created at: {{ formatDatetime(audioForm.createdAt) }} | Updated at:
        {{ formatDatetime(audioForm.updatedAt) }}
      </p>

      <div v-if="audioUpdated" class="alert alert-success" role="alert">
        Audio updated in database.
      </div>
      ...
      <form @submit.prevent="generateAudio">
        ...
        <div class="mb-3">
          ...
          <select id="voiceSelect" class="form-select" v-model="audioForm.voiceName" disabled>
      ...
      <div class="mt-4">
        <button @click="updateAudio" type="button" class="btn btn-primary me-5">
          Update title
        </button>
...
const fetchAudio = async (id) => {
  ...
  downloadFileName.value = createFilename(
    audioForm.value.title,
    audioForm.value.text,
    audioId,
    audioForm.value.createdAt,
  );
};
